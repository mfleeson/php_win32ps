...
setlocal enableextensions enabledelayedexpansion
	...
	for %%z in (%ZTS_STATES%) do (
		...
		cmd /c configure.bat --disable-all --enable-cli --!ZTS_STATE!-zts --enable-win32ps=shared --enable-object-out-dir=%PHP_BUILD_OBJ_DIR% --with-config-file-scan-dir=%APPVEYOR_BUILD_FOLDER%\build\modules.d --with-prefix=%APPVEYOR_BUILD_FOLDER%\build --with-php-build=%DEPS_DIR%
		...
		if not exist "%APPVEYOR_BUILD_FOLDER%\build\ext\php_win32ps.dll" exit /b 3

		rem xcopy %APPVEYOR_BUILD_FOLDER%\LICENSE %APPVEYOR_BUILD_FOLDER%\php_win32ps-%PHP_REL%-!ZTS_SHORT!-%PHP_BUILD_CRT%-%PHP_SDK_ARCH%\ /y /f
		xcopy %APPVEYOR_BUILD_FOLDER%\docs %APPVEYOR_BUILD_FOLDER%\php_win32ps-%PHP_REL%-!ZTS_SHORT!-%PHP_BUILD_CRT%-%PHP_SDK_ARCH%\docs\ /y /f
		...
		move build\ext\php_win32ps.dll artifacts\php_win32ps-%PHP_REL%-!ZTS_SHORT!-%PHP_BUILD_CRT%-%PHP_SDK_ARCH%.dll
	)
endlocal
